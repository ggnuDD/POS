<?php
$DOMAIN = 'http://localhost/POS/src/admin';
?>
<!DOCTYPE html>
<html lang="en">
...
<?php
if (!is_admin()) {
    header("Location: http://localhost/POS/src/client/");
}
?>
...
<body>
    <div class="header">
        <h1> <?php if ($data['render'] == 'order') echo 'Manage Orders';
                else if ($data['render'] == 'account') echo 'Manage Accounts';
                else echo 'Manage Items' ?>
        </h1>
        <a style="position:absolute; right: 50px; top:10px;" href="http://localhost/POS/src/client/Login/Logout"><i class="fa fa-sign-out text-warning" style="font-size: 40px;" aria-hidden="true"></i></a>
    </div>
    <div class="container">
        <div class="row mt-3">
